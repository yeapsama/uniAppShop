	...
	<view class='details'>
		...
		<view class='details-goods'>
			...
			<view class="prices-item">
				...
				<view class='goods-discount'>{{goodsContent.discount}}折</view>
			</view>
		...
		<shareComp :isShow="isShare" :shareInfo="shareInfo" @changeFlag="changeFlag" ref="shareComp"></shareComp>
	</view>
</template>

<script>
	import $http from '@/common/api/request.js'
	import Card from '@/compoments/common/card.vue'
	import CommodityList from '@/compoments/common/commodityList.vue'
	import UniNumberBox from '@/compoments/uni/uni-number-box/uni-number-box.vue'
	import shareComp from"@/compoments/common/share.vue"
	...
	export default {
		...
		onLoad(e) {
			this.getData(e.id);
			// this.shareUrl = `http://192.168.8.6:8080/#/pages/details/details?id=${e.id}`
		},
		//修改返回默认行为
		onBackPress(){
			if(this.isShow){
		...
		onNavigationBarButtonTap(e) {
			if(e.type==='share'){
				this.$refs.shareComp.showPop();
				this.shareInfo.shareUrl= `http://192.168.8.6:8080/#/pages/details/details?id=${this.goodsContent.id}`;
				this.shareInfo.title = this.goodsContent.name;
				this.shareInfo.imgUrl = this.goodsContent.imgUrl;
				// uni.share({
				// 	"provider":"weixin",
				// 	"type":0,
